...
def main():
    ...
    d = ROOT.RDataFrame("Events", "GluGluToRadionToHHTo2G4Q_M300.root")
    print("Entries: {}".format(d.Count().GetValue()))

    ROOT.gInterpreter.Declare("""
    float Get_HighestScore_JetIndex(float evalDNN_BBvsAll, float New_MaxBScore_Jet_pt)
    {
        
        return 42;
    }
    """)
    # d.Define("newvar", 'evalDNN_BBvsAll+New_MaxBScore_Jet_pt' ).Snapshot("Events", "newfile_python.root")
    d.Define("newvar", 'Get_HighestScore_JetIndex(evalDNN_BBvsAll,New_MaxBScore_Jet_pt)' ).Snapshot("Events", "newfile_python2.root")
